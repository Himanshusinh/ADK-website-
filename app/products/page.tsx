      ...
      <section className="py-20 px-6 md:px-20 max-w-[1440px] mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {categories.map((c, idx) => (
            <div
              key={c.slug}
              className="bg-white border border-charcoal/10 hover:border-primary flex flex-col justify-between p-6 shadow-sm hover:shadow-lg transition-all duration-300 group"
            >
              <div>
                <div className="flex justify-between items-center mb-6">
                  <span className="font-mono text-[10px] text-primary">NODE_0{idx + 1}</span>
                  <span className="material-symbols-outlined text-4xl text-charcoal/70 group-hover:text-primary transition-colors">
                    {c.icon}
                  </span>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="border-t border-charcoal/5 pt-4">
                  <span className="font-mono text-[9px] uppercase text-charcoal/40 tracking-wider block mb-2">
                  ...
                  <ul className="space-y-1.5">
                    {c.models.map((m) => (
                      <li key={m.slug}>
                        ...
                        >
                          <span>- {m.name}</span>
                          <span className="text-[9px] bg-tech-blue text-charcoal px-1.5 py-0.5 rounded border border-charcoal/5 font-bold">
                            {m.status}
                          </span>
                        </Link>
                      </li>
                ...
                <Link
                  ...
                  className="w-full border border-charcoal py-3 font-mono text-[10px] uppercase hover:bg-charcoal hover:text-white transition-all tracking-widest flex items-center justify-center gap-1.5 text-center cursor-pointer block font-bold"
                >
